 ...
 * @author afagot
 */
public final class LaborInvoiceItemCache {
	/**
	 * Unique instance of the cache manager.
	 */
	private static LaborInvoiceItemCache instance;

	/**
	 * Maximum number of LaborInvoiceItemBean objects that may be in the cache.
	 */
	private static final int MAX_CACHE_SIZE = 80;

	/**
	 * The number of LaborInvoiceItemBean objects currently in the cache.
	 */
	 ...
	 */
	private LaborInvoiceItemCache() {
	}
	
	/**
	 * Return the unique instance of the class.
	 */
	public static LaborInvoiceItemCache getInstance() {
		if (null == instance) {
			instance = new LaborInvoiceItemCache();
		}
		return instance;
	}

	/**
	 * A LaborInvoiceItemBean object is passed to this method for addition to the cache.
	 */
	public void addLaborInvoiceItem(LaborInvoiceItemBean bean) {
		Integer id = bean.getLaborInvoiceItemId();
		if (null == cache.get(id)) { // bean not in the cache
			...
			if (0 == currentCacheSize) {
				...
				if (MAX_CACHE_SIZE < currentCacheSize) {
					...
					cache.remove(newLink.bean.getLaborInvoiceItemId());
					lru.next = null;
				} else {
			...
			fetchLaborInvoiceItem(id);
		}
	}

	/**
	 * Return the LaborInvoiceItemBean associated with the given key.
	 */
	public LaborInvoiceItemBean fetchLaborInvoiceItem(Integer id) {
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) {
			LaborInvoiceItemBean bean = null;
			try {
				bean = LaborInvoiceItemManager.getInstance().loadByPrimaryKey(id);
			} catch (DAOException e) {
				return null;
			}
			if (null != bean) {
				addLaborInvoiceItem(bean);
			}
			return bean;
		}
		// AFagot [2007-07-06]
