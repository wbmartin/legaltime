 ...
 * @author afagot
 */
public final class ClientAccountRegisterCache {
	/**
	 * Unique instance of the cache manager.
	 */
	private static ClientAccountRegisterCache instance;

	/**
	 * Maximum number of ClientAccountRegisterBean objects that may be in the cache.
	 */
	private static final int MAX_CACHE_SIZE = 80;

	/**
	 * The number of ClientAccountRegisterBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual ClientAccountRegisterBean.
	 */
	private HashMap cache = new HashMap();

	/**
	 * This is the head of the linked list that refers to the most recently used
	 * ClientAccountRegisterBean.
	 */
	 ...
	 */
	private ClientAccountRegisterCache() {
	}
	
	/**
	 * Return the unique instance of the class.
	 */
	public static ClientAccountRegisterCache getInstance() {
		if (null == instance) {
			instance = new ClientAccountRegisterCache();
		}
		return instance;
	}

	/**
	 * A ClientAccountRegisterBean object is passed to this method for addition to the cache.
	 */
	public void addClientAccountRegister(ClientAccountRegisterBean bean) {
		Integer id = bean.getClientAccountRegisterId();
		if (null == cache.get(id)) { // bean not in the cache
			...
			if (0 == currentCacheSize) {
				...
				if (MAX_CACHE_SIZE < currentCacheSize) {
					...
					cache.remove(newLink.bean.getClientAccountRegisterId());
					lru.next = null;
				} else {
			...
			fetchClientAccountRegister(id);
		}
	}

	/**
	 * Return the ClientAccountRegisterBean associated with the given key.
	 */
	public ClientAccountRegisterBean fetchClientAccountRegister(Integer id) {
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) {
			ClientAccountRegisterBean bean = null;
			try {
				bean = ClientAccountRegisterManager.getInstance().loadByPrimaryKey(id);
			} catch (DAOException e) {
				return null;
			}
			if (null != bean) {
				addClientAccountRegister(bean);
			}
			return bean;
		}
		// AFagot [2007-07-06]
	...
	private class LinkedList {
		ClientAccountRegisterBean bean;
		LinkedList previous;
		LinkedList next;
	}
}
