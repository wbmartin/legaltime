 ...
 * @author afagot
 */
public final class InvoiceCache {
	/**
	 * Unique instance of the cache manager.
	 */
	private static InvoiceCache instance;

	/**
	 * Maximum number of InvoiceBean objects that may be in the cache.
	 */
	private static final int MAX_CACHE_SIZE = 80;

	/**
	 * The number of InvoiceBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual InvoiceBean.
	 */
	...
	public static InvoiceCache getInstance() {
		if (null == instance) {
			instance = new InvoiceCache();
		}
		return instance;
	}

	/**
	 * A InvoiceBean object is passed to this method for addition to the cache.
	 */
	public void addInvoice(InvoiceBean bean) {
		Integer id = bean.getInvoiceId();
		if (null == cache.get(id)) { // bean not in the cache
			...
			if (0 == currentCacheSize) {
				...
				if (MAX_CACHE_SIZE < currentCacheSize) {
					...
					cache.remove(newLink.bean.getInvoiceId());
					lru.next = null;
				} else {
	...
	/**
	 * Return the InvoiceBean associated with the given key.
	 */
	public InvoiceBean fetchInvoice(Integer id) {
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) {
			InvoiceBean bean = null;
			try {
				bean = InvoiceManager.getInstance().loadByPrimaryKey(id);
