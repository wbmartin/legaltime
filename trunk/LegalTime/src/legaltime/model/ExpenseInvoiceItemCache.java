 ...
 * @author afagot
 */
public final class ExpenseInvoiceItemCache {
	/**
	 * Unique instance of the cache manager.
	 */
	private static ExpenseInvoiceItemCache instance;

	/**
	 * Maximum number of ExpenseInvoiceItemBean objects that may be in the cache.
	 */
	private static final int MAX_CACHE_SIZE = 80;

	/**
	 * The number of ExpenseInvoiceItemBean objects currently in the cache.
	 */
	 ...
	 */
	private ExpenseInvoiceItemCache() {
	}
	
	/**
	 * Return the unique instance of the class.
	 */
	public static ExpenseInvoiceItemCache getInstance() {
		if (null == instance) {
			instance = new ExpenseInvoiceItemCache();
		}
		return instance;
	}

	/**
	 * A ExpenseInvoiceItemBean object is passed to this method for addition to the cache.
	 */
	public void addExpenseInvoiceItem(ExpenseInvoiceItemBean bean) {
		Integer id = bean.getExpenseInvoiceItemId();
		if (null == cache.get(id)) { // bean not in the cache
			...
			if (0 == currentCacheSize) {
				...
				if (MAX_CACHE_SIZE < currentCacheSize) {
					...
					cache.remove(newLink.bean.getExpenseInvoiceItemId());
					lru.next = null;
				} else {
			...
			fetchExpenseInvoiceItem(id);
		}
	}

	/**
	 * Return the ExpenseInvoiceItemBean associated with the given key.
	 */
	public ExpenseInvoiceItemBean fetchExpenseInvoiceItem(Integer id) {
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) {
			ExpenseInvoiceItemBean bean = null;
			try {
				bean = ExpenseInvoiceItemManager.getInstance().loadByPrimaryKey(id);
			} catch (DAOException e) {
				return null;
			}
			if (null != bean) {
				addExpenseInvoiceItem(bean);
			}
			return bean;
		}
		// AFagot [2007-07-06]
