 ...
 * @author afagot
 */
public final class ExpenseRegisterCache {
	/**
	 * Unique instance of the cache manager.
	 */
	private static ExpenseRegisterCache instance;

	/**
	 * Maximum number of ExpenseRegisterBean objects that may be in the cache.
	 */
	private static final int MAX_CACHE_SIZE = 80;

	/**
	 * The number of ExpenseRegisterBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual ExpenseRegisterBean.
	 */
	 ...
	 */
	private ExpenseRegisterCache() {
	}
	
	/**
	 * Return the unique instance of the class.
	 */
	public static ExpenseRegisterCache getInstance() {
		if (null == instance) {
			instance = new ExpenseRegisterCache();
		}
		return instance;
	}

	/**
	 * A ExpenseRegisterBean object is passed to this method for addition to the cache.
	 */
	public void addExpenseRegister(ExpenseRegisterBean bean) {
		Integer id = bean.getExpenseRegisterId();
		if (null == cache.get(id)) { // bean not in the cache
			...
			if (0 == currentCacheSize) {
				...
				if (MAX_CACHE_SIZE < currentCacheSize) {
					...
					cache.remove(newLink.bean.getExpenseRegisterId());
					lru.next = null;
				} else {
			...
			fetchExpenseRegister(id);
		}
	}

	/**
	 * Return the ExpenseRegisterBean associated with the given key.
	 */
	public ExpenseRegisterBean fetchExpenseRegister(Integer id) {
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) {
			ExpenseRegisterBean bean = null;
			try {
				bean = ExpenseRegisterManager.getInstance().loadByPrimaryKey(id);
			} catch (DAOException e) {
				return null;
			}
			if (null != bean) {
				addExpenseRegister(bean);
			}
			return bean;
		}
		// AFagot [2007-07-06]
