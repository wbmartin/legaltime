...
public final class UserInfoCache {
	...
	/**
	 * Maximum number of UserInfoBean objects that may be in the cache.
	 */
	private static final int MAX_CACHE_SIZE = 80;

	/**
	 * The number of UserInfoBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual UserInfoBean.
	 */
	...
	/**
	 * A UserInfoBean object is passed to this method for addition to the cache.
	 */
	public void addUserInfo(UserInfoBean bean) {
		String id = bean.getUserKey();
		if (null == cache.get(id)) { // bean not in the cache
			...
			if (0 == currentCacheSize) {
				...
				if (MAX_CACHE_SIZE < currentCacheSize) {
					...
					cache.remove(newLink.bean.getUserKey());
					lru.next = null;
				} else {
	...
	/**
	 * Return the UserInfoBean associated with the given key.
	 */
	public UserInfoBean fetchUserInfo(String id) {
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) {
			UserInfoBean bean = null;
			try {
				bean = UserInfoManager.getInstance().loadByPrimaryKey(id);
